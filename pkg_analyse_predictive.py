...
API_SCRIPT = str(BASE_DIR / "blood_eligibility_api.py")  # Nom du fichier principal de l'API
PID_FILE = str(BASE_DIR / "api_pid.txt")
LOG_DIR = str(BASE_DIR / "logs")
API_LOG = str(Path(LOG_DIR) / "api.log")
MONITOR_LOG = str(Path(LOG_DIR) / "monitor.log")

# Configuration de l'API
API_HOST = "0.0.0.0"  # Adresse de l'hôte (0.0.0.0 pour être accessible depuis l'extérieur)
API_PORT = 10000  # Port sur lequel l'API sera accessible
CHECK_INTERVAL = 60  # Vérifier l'état de l'API toutes les X secondes
MAX_RESTART_ATTEMPTS = 3  # Nombre maximum de tentatives de redémarrage par heure
HEALTH_ENDPOINT = f"https://api-blood-donnation.onrender.com/docs"  # Point de terminaison de vérification de santé
...
def save_pid(pid):
    """Sauvegarde l'ID du processus dans un fichier"""
    try:
        with open(PID_FILE, "w") as f:
            f.write(str(pid))
        logger.info(f"PID {pid} enregistré dans {PID_FILE}")
    except Exception as e:
        logger.error(f"Erreur lors de l'enregistrement du PID: {e}")

def read_pid():
    """Lit l'ID du processus à partir du fichier"""
    try:
        if os.path.exists(PID_FILE):
            with open(PID_FILE, "r") as f:
                return int(f.read().strip())
    except Exception as e:
        logger.error(f"Erreur lors de la lecture du PID: {e}")
    return None
...
def is_api_running():
    """Vérifie si l'API est en cours d'exécution en faisant une requête à son endpoint de santé"""
    try:
        response = requests.get(HEALTH_ENDPOINT, timeout=5)
        if response.status_code == 200:
            status_data = response.json()
            logger.debug(f"Statut de l'API: {status_data.get('status', 'unknown')}")
            return True
        return False
...
def kill_existing_process():
    """Tue tout processus existant de l'API"""
    pid = read_pid()
    if pid:
        try:
            os.kill(pid, signal.SIGTERM)
            logger.info(f"Signal SIGTERM envoyé au processus {pid}")
            time.sleep(2)  # Attendre que le processus se termine proprement
            
            # Vérifier si le processus existe toujours
            try:
                os.kill(pid, 0)  # Vérifier si le processus existe encore
                # S'il n'y a pas d'exception, le processus existe toujours
                logger.warning(f"Le processus {pid} existe encore, envoi de SIGKILL")
                os.kill(pid, signal.SIGKILL)  # Force kill
            except OSError:
                logger.info(f"Le processus {pid} s'est terminé normalement")
            ...
            if e.errno == 3:  # "No such process"
                logger.warning(f"Le processus {pid} n'existe plus")
                if os.path.exists(PID_FILE):
                    os.remove(PID_FILE)
            else:
                logger.error(f"Erreur lors de la tentative de tuer le processus {pid}: {e}")

def start_api():
    """Démarre l'API comme un processus séparé"""
    global api_process, restart_attempts, last_restart_hour
    
    # Vérifier si nous avons dépassé le nombre maximum de tentatives de redémarrage pour l'heure en cours
    ...
    if restart_attempts >= MAX_RESTART_ATTEMPTS:
        logger.error(f"Nombre maximum de tentatives de redémarrage atteint ({MAX_RESTART_ATTEMPTS}) pour cette heure. Attente jusqu'à la prochaine heure.")
        return False
    
    try:
        # Vérifier si le port est déjà utilisé
        if is_port_in_use(API_PORT):
            logger.warning(f"Le port {API_PORT} est déjà utilisé. Tentative de libération...")
            kill_existing_process()
            time.sleep(2)  # Attendre que le port soit libéré
            
            if is_port_in_use(API_PORT):
                logger.error(f"Impossible de libérer le port {API_PORT}. Abandon du démarrage.")
                return False
        
        # Créer la commande uvicorn avec les bons paramètres
        # Supposons que le fichier d'API s'appelle "blood_eligibility_api.py" et 
        # que l'application FastAPI s'appelle "app_fastapi"
        cmd = [
            sys.executable, "-m", "uvicorn", 
            "blood_eligibility_api:app_fastapi", 
            "--host", API_HOST, 
            "--port", str(API_PORT),
            "--log-level", "info"
        ]
        
        # Rediriger stdout et stderr vers le fichier de log de l'API
        log_file = open(API_LOG, "a")
        
        # Démarrer le processus
        logger.info(f"Démarrage de l'API avec la commande: {' '.join(cmd)}")
        api_process = subprocess.Popen(
            cmd,
            stdout=log_file,
            stderr=log_file,
            cwd=BASE_DIR  # Définir le répertoire de travail
        )
        
        # Enregistrer le PID
        save_pid(api_process.pid)
        ...
        for i in range(10):  # Attendre jusqu'à 10 x 2 = 20 secondes
            time.sleep(2)
            if is_api_running():
                logger.info("L'API a démarré avec succès!")
                return True
            logger.debug(f"Tentative {i+1}/10 de vérification du démarrage de l'API...")
        
        logger.error("L'API n'a pas pu démarrer dans le délai imparti.")
        return False
...
def cleanup():
    ...
    logger.info("Arrêt du moniteur, nettoyage des ressources...")
    if api_process:
        try:
            api_process.terminate()
            logger.info(f"Signal terminate envoyé au processus API (PID: {api_process.pid})")
        except Exception as e:
            logger.error(f"Erreur lors de la terminaison du processus API: {e}")

def main():
    """Fonction principale qui surveille et gère l'API"""
    global restart_attempts, last_restart_hour
    
    logger.info("=== Démarrage du moniteur d'API ===")
    logger.info(f"Chemin de l'API: {API_SCRIPT}")
    ...
    # Initialiser le suivi des tentatives de redémarrage
    restart_attempts = 0
    last_restart_hour = datetime.now().hour
    
    # Tuer tout processus existant au démarrage
    kill_existing_process()
    
    # Au démarrage du script, lancer l'API
    if not start_api():
        logger.error("Impossible de démarrer l'API au lancement du moniteur. Nouvelle tentative dans le prochain cycle.")
    ...
    try:
        while True:
            try:
                if not is_api_running():
                    logger.warning("L'API n'est pas en cours d'exécution. Tentative de redémarrage...")
                    if not start_api():
                        logger.error("Échec du redémarrage de l'API.")
                else:
                    logger.debug("L'API fonctionne correctement.")
            except Exception as e:
        ...
        logger.info("Interruption du moniteur par l'utilisateur (Ctrl+C)")
    finally:
        cleanup()
    ...
    if statut_predit == "Définitivement non-eligible":
        ...
        if data.Date_dernier_don and data.A_deja_donne == "Oui":
            ...
            if (current_date - last_donation_date).days < 90:
                raisons.append("La date de votre dernier est inférieur à trois mois")
                recommandations.append("Veuillez passer après trois mois écoulés par rapport à la date de votre dernier don")
